/*
 * SPDX-FileCopyrightText: 2023-2024 Espressif Systems (Shanghai) CO LTD
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include "soc/soc_caps.h"
#include "esp_check.h"
#include "esp_lcd_types.h"
#include "esp_lcd_panel_interface.h"

#include "esp_lcd_sh8601.h"
#include "esp_lcd_sh8601_interface.h"

static const char *TAG = "sh8601";

esp_err_t esp_lcd_new_panel_sh8601(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config,
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ESP_RETURN_ON_FALSE(panel_dev_config && ret_panel, ESP_ERR_INVALID_ARG, TAG, "Invalid arguments");
    sh8601_vendor_config_t *vendor_config = NULL;
    if (panel_dev_config->vendor_config) {
        vendor_config = (sh8601_vendor_config_t *)panel_dev_config->vendor_config;
    }

    esp_err_t ret = ESP_ERR_NOT_SUPPORTED;

#if SOC_MIPI_DSI_SUPPORTED
    if (vendor_config && vendor_config->flags.use_mipi_interface) {
        ret = esp_lcd_new_panel_sh8601_mipi(io, panel_dev_config, ret_panel);

        return ret;
    }
#endif
#if !SOC_MIPI_DSI_SUPPORTED
    (void)vendor_config;
#endif

    ret = esp_lcd_new_panel_sh8601_spi(io, panel_dev_config, ret_panel);

    return ret;
}

esp_err_t esp_lcd_panel_sh8601_set_brightness(esp_lcd_panel_handle_t panel, uint8_t brightness_percent)
{
    ESP_RETURN_ON_FALSE(panel, ESP_ERR_INVALID_ARG, TAG, "invalid panel handle");
    ESP_RETURN_ON_FALSE(brightness_percent <= 100, ESP_ERR_INVALID_ARG, TAG, "brightness percentage must be 0-100");

    sh8601_panel_context_t *panel_ctx = (sh8601_panel_context_t *)panel->user_data;
    ESP_RETURN_ON_FALSE(panel_ctx, ESP_ERR_INVALID_STATE, TAG, "panel context not initialized");
    ESP_RETURN_ON_FALSE(panel_ctx->apply_brightness, ESP_ERR_NOT_SUPPORTED, TAG, "brightness control not supported");

    ESP_RETURN_ON_ERROR(panel_ctx->apply_brightness(panel_ctx->driver_data, brightness_percent), TAG,
                        "apply brightness failed");

    return ESP_OK;
}
