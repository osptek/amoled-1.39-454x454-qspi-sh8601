 ...
 */
typedef struct {
    const sh8601_lcd_init_cmd_t *init_cmds;         /*!< Pointer to initialization commands array. Set to NULL if using default commands.
 ...
 *
 * @note  When `enable_io_multiplex` is set to 1, this function will first initialize the SH8601 with vendor specific initialization and then calls `esp_lcd_new_rgb_panel()` to create an RGB LCD panel. And the `esp_lcd_panel_init()` function will only initialize RGB.
 * @note  When `enable_io_multiplex` is set to 0, this function will only call `esp_lcd_new_rgb_panel()` to create an RGB LCD panel. And the `esp_lcd_panel_init()` function will initialize both the SH8601 and RGB.
 ...
 *      - Otherwise             on fail
 */
esp_err_t esp_lcd_panel_sh8601_set_brightness(esp_lcd_panel_handle_t panel, uint8_t brightness_percent);
 ...
 */
#define SH8601_PANEL_BUS_DSI_1CH_CONFIG()                 \
    {                                                     \
        .bus_id = 0,                                      \
        .num_data_lanes = 1,                              \
        .phy_clk_src = 0,                                 \
        .lane_bit_rate_mbps = 420,                        \
    }
 ...
 *
 */
#define SH8601_PANEL_IO_DBI_CONFIG() \
    {                                 \
        .virtual_channel = 0,         \
        .lcd_cmd_bits = 8,            \
        .lcd_param_bits = 8,          \
 ...
 *
 */
#define SH8601_410_502_PANEL_60HZ_DPI_CONFIG(px_format)          \
    {                                                            \
