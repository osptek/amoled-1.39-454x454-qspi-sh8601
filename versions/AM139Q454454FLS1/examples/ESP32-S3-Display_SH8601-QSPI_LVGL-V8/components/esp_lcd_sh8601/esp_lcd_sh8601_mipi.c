...
#include "esp_lcd_sh8601_interface.h"
...
typedef struct {
    ...
    sh8601_panel_context_t panel_ctx; // runtime context shared with public API
...
static const char *TAG = "sh8601_mipi";
...
static esp_err_t sh8601_mipi_apply_brightness(void *driver_data, uint8_t brightness_percent);

esp_err_t esp_lcd_new_panel_sh8601_mipi(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config,
                                        esp_lcd_panel_handle_t *ret_panel)
{
    ...
    sh8601->del = panel_handle->del;
    sh8601->init = panel_handle->init;
    // Overwrite the functions of MIPI DPI panel
    panel_handle->del = panel_sh8601_del;
    panel_handle->init = panel_sh8601_init;
    panel_handle->reset = panel_sh8601_reset;
    sh8601->panel_ctx.driver_data = sh8601;
    sh8601->panel_ctx.apply_brightness = sh8601_mipi_apply_brightness;
    panel_handle->user_data = &sh8601->panel_ctx;
...
static const sh8601_lcd_init_cmd_t vendor_specific_init_code_default[] = {
...
static esp_err_t panel_sh8601_del(esp_lcd_panel_t *panel)
{
    sh8601_panel_context_t *panel_ctx = (sh8601_panel_context_t *)panel->user_data;
    ESP_RETURN_ON_FALSE(panel_ctx, ESP_ERR_INVALID_STATE, TAG, "panel context not initialized");
    sh8601_panel_t *sh8601 = (sh8601_panel_t *)panel_ctx->driver_data;
    ESP_RETURN_ON_FALSE(sh8601, ESP_ERR_INVALID_STATE, TAG, "invalid panel data");

    // Delete MIPI DPI panel
    ESP_RETURN_ON_ERROR(sh8601->del(panel), TAG, "del sh8601 panel failed");
    if (sh8601->reset_gpio_num >= 0) {
        gpio_reset_pin(sh8601->reset_gpio_num);
    }

    if (sh8601->del) {
        ESP_RETURN_ON_ERROR(sh8601->del(panel), TAG, "delete MIPI DPI panel failed");
    }
...
static esp_err_t panel_sh8601_init(esp_lcd_panel_t *panel)
{
    ...
    if (sh8601->init_cmds) {
        ...
        init_cmds_size = sizeof(vendor_specific_init_code_default) / sizeof(sh8601_lcd_init_cmd_t);
    }
    ...
    if (sh8601->init) {
        ESP_RETURN_ON_ERROR(sh8601->init(panel), TAG, "init MIPI DPI panel failed");
    }
...
static esp_err_t sh8601_mipi_apply_brightness(void *driver_data, uint8_t brightness_percent)
{
    sh8601_panel_t *sh8601 = (sh8601_panel_t *)driver_data;
    ESP_RETURN_ON_FALSE(sh8601, ESP_ERR_INVALID_ARG, TAG, "invalid panel data");
